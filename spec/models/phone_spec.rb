...
RSpec.describe Phone, type: :model do
  ...
  describe 'validations' do
    it { expect(subject).to allow_value(true, false).for(:blacklist) }
    ...
    it { expect(subject).to_not allow_value('123-456-7890').for(:number).with_message("only allows numbers") }
  end
end